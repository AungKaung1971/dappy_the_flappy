# rolloutbuffer script observing every game
...
class RolloutBuffer:
    def __init__(self, buffer_size, obs_shape, device):
        ...
        if self.ptr >= self.buffer_size:
            raise IndexError(
                "RolloutBuffer is full. Call reset() before storing more data.")
    ...
    def compute_returns_and_advantages(self, last_value, gamma=0.99, lam=0.95):
        if self.ptr != self.buffer_size:
            raise ValueError(
                ...
                "Make sure you collected buffer_size steps before calling compute_returns_and_advantages()."
            )
    ...
    def get(self, normalize_advantages=True):
        if self.ptr != self.buffer_size:
            raise ValueError(
                f"Trying to get data from buffer, but ptr={self.ptr} != buffer_size={self.buffer_size}"
            )

        observations = torch.from_numpy(self.observations).to(self.device)
        ...
        return observations, actions, old_logprobs, returns, advantages, values
